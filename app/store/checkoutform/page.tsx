...
export default function CheckoutForm() {
  const handleStripeCheckout = async () => {
    const session = await createStripeCheckoutSession([
      { name: "Handmade Bag", price: 2500, quantity: 1 },
    ]);
    await redirectToStripeCheckout(session.id);
  };

  const handleRazorpayCheckout = async () => {
    const order = await createRazorpayOrder(2500, "INR");
  ...
  return (
    <div>
      ...
      <button onClick={handleRazorpayCheckout}>Pay with Razorpay</button>
    </div>
  );
}
...
// // TODO: Replace with your real cart store / context
// const mockCart = [
//   { id: "1", name: "Flower Pot", price: 100, quantity: 2, image: "https://via.placeholder.com/150" },
//   { id: "2", name: "Kulhad Tea Cup", price: 50, quantity: 4, image: "https://via.placeholder.com/150" },
// ];
...
//                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                   <InputSimple label="Card Number" name="cardNumber" value={formData.cardNumber} onChange={handleChange} />
//                   <InputSimple label="Expiry Date" name="expiryDate" value={formData.expiryDate} onChange={handleChange} placeholder="MM/YY" />
//                   <InputSimple label="CVV" name="cvv" value={formData.cvv} onChange={handleChange} placeholder="123" />
//                   <InputSimple label="Cardholder Name" name="cardName" value={formData.cardName} onChange={handleChange} className="md:col-span-2" />
//                 </div>
//               )}
//             </div>
//           </div>
...
// const InputWithIcon = ({ label, name, value, onChange, icon, type = "text" }: any) => (
//   <div>
...
// const InputSimple = ({ label, name, value, onChange, type = "text", placeholder = "", className = "" }: any) => (
...
// const SummaryRow = ({ label, value, bold = false, highlight = false }: any) => (
